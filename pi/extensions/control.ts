/**
 * Vendored from mitsuhiko/agent-stuff (Apache-2.0), commit 13bc8f87970bec8830aab0f1c0487d35aa7c0917
 * https://github.com/mitsuhiko/agent-stuff/blob/main/extensions/control.ts
 * Diff against that path to check for upstream drift.
 *
 * Local change: CONTROL_DIR now uses pi's getAgentDir() instead of a
 * hardcoded ~/.pi, so it respects PI_CODING_AGENT_DIR.
 */
/**
...
const CONTROL_DIR = path.join(getAgentDir(), "session-control"); // was: path.join(os.homedir(), ".pi", "session-control")
